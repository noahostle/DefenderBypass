...
char* fetch_url_response(const char* url) {
    ...
    memset(response, 0, 10000);  // Initialize the response buffer
...
int main() {
    ...
    char* response = fetch_url_response(url);
    
    if (response) {
        printf("Response:\n%s\n", response);
        free(response);  // Free the response memory
    } else {
